...
After completion of this repository's README, the following packages are already installed. The source code and structure of the packages is provided by Hiwonder. The structure is somewhat confusing, so here is a small explanation of the functionality of each package. 
...
## driver
This folder contains the code necessary to control the robot. The code is structured like this:
```
|-- driver
    |-- controller
    ...
    |-- sdk
```
Each of the four subfolders are packages. The `/controller` package contains the files that are necessary for robot startup. The `/ros_robot_controller`, `/ros_robot_controller_msgs` and `/sdk` folders are used for communication with the extension board. None of these files should need to be modified.
